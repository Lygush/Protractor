   ...
   public:
    ...
    void writeData(byte data, byte offsetY = 0, byte offsetX = 0, int mode = 0) {
        if (_BUFF) {
            ...
            if (_buf_flag) {
                int x = _x - _bufX + offsetX;
                int y = _y - _bufY;
                int page = (y >> 3) + offsetY;
                if (x < 0 || x >= _bufsizeX || y < 0 || page >= _bufsizeY) return;
                switch (mode) {
                    case 0:
                        _buf_ptr[page + x * _bufsizeY] |= data;  // добавить
                        break;
                    case 1:
                        _buf_ptr[page + x * _bufsizeY] &= ~data;  // вычесть
                        break;
                    case 2:
                        _buf_ptr[page + x * _bufsizeY] = data;  // заменить
                        break;
                }
            } else {
                sendByte(data);
