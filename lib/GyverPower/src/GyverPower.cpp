...
#if defined(PRR0) && defined(PRR1)
    PRR1 &= ~highByte(data);  // загрузили данные в регистр
    PRR0 &= ~lowByte(data);
#elif defined(PRR1)
    // Некоторые ядра (например framework-lgt8fx для LGT8F328P) определяют
    // только PRR1, без PRR0 — в отличие от ATmega328PB, где регистров два.
    // В этом случае весь функционал регулировки периферии живёт в PRR1.
    PRR1 &= ~lowByte(data);
#else
    PRR &= ~lowByte(data);
#endif
...
#elif defined(PRR1)
    if (!(PRR1 & (1 << PRADC))) {  // если ацп не выключен принудительно
        ADCSRA |= (1 << ADEN);     // вкл после сна
        ACSR &= ~(1 << ACD);
    }
#else
